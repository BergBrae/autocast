...
class XPrimeStreamAPI(VideoSourceAPI):
    ...
    async def search_streams(self, metadata: MediaMetadata, original_request: VideoRequest) -> VideoSources:
        ...
        print(f"[{self.name}] Searching for streams for: '{movie_name}'")
        ...
        try:
            ...
            if data.get("status") == "ok" and "streams" in data and data["streams"]:
                ...
                for quality in preferred_qualities:
                    if quality in available_streams:
                        ...
                        print(f"[{self.name}] Added {quality} stream: {stream_url[:60]}...")
            ...
            if e.response.status_code == 429:
                print(f"[{self.name}] Rate limited (HTTP 429). Try again in a few seconds.")
                print(f"[{self.name}] Note: xprime.tv has rate limiting. Consider waiting between requests.")
            else:
    ...
    async def test_xprime():
        ...
        if not omdb_api_key:
            ...
            async with httpx.AsyncClient() as client:
                test_request = VideoRequest(title="The Matrix", destination_tv="any")
                test_metadata = await get_media_metadata(omdb_api_key, test_request, client)
